...
@onready var _collision_player := $"../CollisionShape2D" # Colicionador de personaje
...
var _movements = {
	...
	ATTACK = "attack_2",
	BOMB = "attack_3",
	CROUCH = "crouch_with_sword", # nuevo metodo de agacharse
}
...
func _move(delta):
	...
	# Cuando se presiona la tecla (flecha derecha), movemos el personaje a la derecha
		...
		turn_side = "right"
	# Cuando presionamos la tecla abajo	
	elif Input.is_action_pressed("abajo"):
		character.velocity.x = 0
		_current_movement = _movements.CROUCH
...
func _set_animation():
	...
	if _is_jumping:	
	...
	elif _current_movement == _movements.CROUCH:
		main_animation.play(_movements.CROUCH)
		_collision_player.scale.y = (_collision_player.scale.y)/2
	else:
		...
		audio_player.stop()
		_is_playing = ""
		#devolvemos a la escala normal
		_collision_player.scale = Vector2(1,1)
